...
block_length = 10
model = load('trained_model/nn1.joblib')


def fill_square(canvas, i, j):
    n, d = canvas.shape
    if 0 <= i < n and 0 <= j < d:
        canvas[i][j] = 1
...
def compress(canvas):
    ...
    for i in range(28):
        for j in range(28):
            retVal[i][j] = (canvas[2 * i][2 * j] + canvas[2 * i + 1][2 * j] + canvas[2 * i][2 * j + 1] +
...
def main():
    while True:
        ...
        if pygame.mouse.get_pressed()[0]:
            i = pygame.mouse.get_pos()[1] // block_length
            j = pygame.mouse.get_pos()[0] // block_length
            for y in [i - 1, i, i + 1]:
                for x in [j - 1, j, j + 1]:
                    fill_square(canvas, y, x)
